    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg dark:hover:shadow-gray-900/50 transition-all duration-500 border border-gray-100 dark:border-gray-800 flex flex-col group p-4 h-full">
      ...
      <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 mb-4 transition-colors duration-500 flex-shrink-0">
        <img 
          ...
          // Se estiver vendido, a imagem fica ligeiramente cinzenta e não faz zoom ao passar o rato
          className={`w-full h-full object-cover transition-transform duration-700 ${vehicle.em_stock ? 'group-hover:scale-105' : 'grayscale-[40%]'}`}
        />
        
        {vehicle.em_destaque && vehicle.em_stock && (
        ...
        {/* ETIQUETA DE VENDIDO E EFEITO VISUAL */}
        {!vehicle.em_stock && (
          <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] z-10 flex items-center justify-center opacity-80">
            <div className="bg-red-600 text-white font-black uppercase tracking-widest px-6 py-2 rounded-xl text-lg shadow-2xl -rotate-12 border-2 border-red-500/50 opacity-100">
              Vendido
            </div>
          </div>
        )}
